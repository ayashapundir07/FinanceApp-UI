...
import { DollarIcon, FreelancingIcon, InterestIcon } from "@/constants/Icons";
...
const IncomeBlock = ({ incomeList }: { incomeList: IncomeType[] }) => {
  const renderItem: ListRenderItem<IncomeType> = ({ item }) => {
    ...
    if (item.name == "Freelancing") {
      icon = <FreelancingIcon width={22} height={22} color={Colors.white} />;
    } else if (item.name == "Interest") {
      icon = <InterestIcon width={22} height={22} color={Colors.white} />;
    }
